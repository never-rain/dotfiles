...
2. Set up the Griffo APT repository. When asked whether you have subscription credentials, answer `n` to use the public repository, or Enter to use the paid repository. For the paid repository, enter your username and password; password input is hidden. Existing credentials can be kept (Enter) or replaced (`n`). Running the script again with credentials switches an existing public source to the paid one.
...
Existing GitHub CLI entries are detected and left in place. For Griffo, the script changes URLs in standard APT `.list` and `.sources` files to the selected public or paid host, including legacy `debian.griffo.io/apt` entries. Other sources and existing suites and signing-key settings are preserved; changed files get a `.griffo-backup` copy. An existing active source is reused instead of adding another entry. Both Griffo repositories use the same signing key. This does not verify an existing key or repair other configuration problems. The script uses `sudo` for package and repository changes; `chsh` and Stow run as your normal user. The shell step checks the account entry and `/etc/shells`, then verifies the new account entry after `chsh` succeeds.
