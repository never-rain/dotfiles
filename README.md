...
10. Choose a final action: `0`/Enter keeps you logged in, `1` ends the current session with `loginctl terminate-session`, and `2` requests a normal system reboot with `systemctl reboot`. Save open work before choosing logout or reboot. If the session cannot be identified, logout is not attempted. These choices appear only after the preceding steps have completed or been skipped.
