...
6. Install Zap for Zsh by cloning its `release-v1` branch into `${XDG_DATA_HOME:-$HOME/.local/share}/zap`. An existing Zap directory is skipped. This does not modify or source `.zshrc`, so installation also works before Stow has linked your shell configuration.
7. Link all Stow packages into your home directory. The script lists the packages and target before asking for confirmation, then checks for conflicts with a simulation before creating links.
