...
7. Install [Codex CLI](https://learn.chatgpt.com/docs/codex/cli) with `pnpm add --global @openai/codex`, without `sudo`. An existing `codex` command is skipped. If pnpm is unavailable, this step is skipped with a message. pnpm is available immediately after step 6; no new terminal is needed. The script verifies the installed CLI with `--version`; sign in separately when you first run `codex`.
